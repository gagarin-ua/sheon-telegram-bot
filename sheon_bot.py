...
async def main():
    ...
    try:
        ...
        # 4.1. ДОДАТКОВА ДІАГНОСТИКА: Перевірка статусу вебхука
        webhook_info = await application.bot.get_webhook_info()
        logger.info(f"🌐 Поточний Webhook URL за даними Telegram: {webhook_info.url}")
        if not webhook_info.url.startswith(WEBHOOK_URL_BASE):
             logger.warning(f"⚠️ Увага! URL вебхука, встановлений на Telegram, не відповідає очікуваному базовому URL: {WEBHOOK_URL_BASE}")
        else:
            logger.info("👍 Webhook URL відповідає очікуваному базовому URL.")

    except Exception as e:
        logger.error(f"❌ Помилка при встановленні Webhook на Telegram: {e}")
        # Зупиняємо виконання, якщо не вдалося встановити вебхук, оскільки бот не зможе приймати оновлення
        sys.exit(1)
